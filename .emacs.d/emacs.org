...
(define-key evil-visual-state-map (kbd ", r") (lambda () (interactive) (ol-replace-visual-selection)))

(defun ol-replace-visual-selection ()
  (let ((selection (buffer-substring-no-properties (mark) (point))))
    (ol-replace-selection selection)))

(defun ol-replace-selection (selection)
  (let ((ex-command (format ",$s/%s/%s/gc" selection selection)))
...
- Word definition for elisp files
