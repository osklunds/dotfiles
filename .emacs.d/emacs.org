...
#+begin_src emacs-lisp
  ...
  (defun msk-merge-survival-knife-start ()
    ...
    (let* ((file (magit-current-file))
	   ...
	   (buffer-base-bl (msk--get-revision-buffer rev-base file-base))
	   (buffer-base-br (msk--clone-buffer buffer-base-bl))
	   
	   (buffer-local-bl (msk--get-revision-buffer rev-local file-local))
	   (buffer-local-lm (msk--clone-buffer buffer-local-bl))
	   
	   (buffer-remote-br (msk--get-revision-buffer rev-remote file-remote))
	   (buffer-remote-rm (msk--clone-buffer buffer-remote-br))
	   
	   (buffer-merged-lm (current-buffer))
	   (buffer-merged-rm (msk--clone-buffer buffer-merged-lm))

	   )

      (message "hej")

      (vdiff-buffers buffer-base-bl buffer-local-bl)
      (vdiff-buffers buffer-base-br buffer-remote-br)
      (vdiff-buffers buffer-local-lm buffer-merged-lm)
      (vdiff-buffers buffer-remote-rm buffer-merged-rm)
      
      (msk--put-value 'buffer-base-bl buffer-base-bl)
      (msk--put-value 'buffer-base-br buffer-base-br)

      (msk--put-value 'buffer-local-bl buffer-local-bl)
      (msk--put-value 'buffer-local-lm buffer-local-lm)

      (msk--put-value 'buffer-remote-br buffer-remote-br)
      (msk--put-value 'buffer-remote-rm buffer-remote-rm)

      (msk--put-value 'buffer-merged-rm buffer-merged-rm)
  ...
  (defun msk-merge-survival-knife-stop ()
    ...
    (kill-buffer (msk--get-value 'buffer-base-bl))
    (kill-buffer (msk--get-value 'buffer-base-br))
    
    (kill-buffer (msk--get-value 'buffer-local-bl))
    (kill-buffer (msk--get-value 'buffer-local-lm))

    (kill-buffer (msk--get-value 'buffer-remote-br))
    (kill-buffer (msk--get-value 'buffer-remote-rm))

    (kill-buffer (msk--get-value 'buffer-merged-rm))
  ...
  (defun msk-base-local ()
    (interactive)
    (msk--compare-buffer-pair 'buffer-base-bl 'buffer-local-bl))

  (defun msk-base-remote ()
    (interactive)
    (msk--compare-buffer-pair 'buffer-base-br 'buffer-remote-br))

  (defun msk-local-remote ()
    (interactive)
    (msk--compare-buffer-pair 'buffer-local))

  (defun msk-local-merged ()
    (interactive)
    (msk--compare-buffer-pair 'buffer-local-lm 'buffer-merged-lm))

  (defun msk-remote-merged ()
    (interactive)
    (msk--compare-buffer-pair 'buffer-remote-rm 'buffer-merged-rm))

  (defun msk--compare-buffer-pair (buffer-left buffer-right)
    (delete-other-windows)
    (switch-to-buffer (msk--get-value buffer-right))
    (split-window-right)
    (switch-to-buffer (msk--get-value buffer-left)))

  (defun msk--get-revision-buffer (rev file)
    (magit-get-revision-buffer rev file (magit-find-file-noselect rev file)))

  (defun msk--clone-buffer (original-buffer)
    (let* ((name (format "CLONE %s" (buffer-name original-buffer))))
      (make-indirect-buffer original-buffer name)))
