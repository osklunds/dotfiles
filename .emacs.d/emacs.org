...
#+begin_src emacs-lisp
  ...
  (defvar ol-merge-survival-knife-buffers ())

  ;; Copied and modified from magit. TODO: Copyright
  (defun ol-merge-survival-knife-start ()
    (interactive)
    ;; TODO: Check if () first
    (setq ol-merge-survival-knife-buffers ())
    (let* ((file (magit-current-file))
	   ...
	   (bufferBaseLocal (ol--ediff bufferBase bufferLocal "BASE LOCAL"))
   	   (bufferBaseRemote (ol--ediff bufferBase bufferRemote "BASE REMOTE"))
	   (bufferLocalRemote (ol--ediff bufferLocal bufferRemote "LOCAL REMOTE"))
	   (bufferLocalMerged (ol--ediff bufferLocal bufferMerged "LOCAL MERGED"))
   	   (bufferRemoteMerged (ol--ediff bufferRemote bufferMerged "REMOTE MERGED")))
      
      ;; (message "EDIFF %s" bufferBaseLocal)

      (ol--put-buffer 'base bufferBase)
      (ol--put-buffer 'local bufferLocal)
      (ol--put-buffer 'remote bufferRemote)
      
      (ol--put-buffer 'baseLocal bufferBaseLocal)
      (ol--put-buffer 'baseRemote bufferBaseRemote)
      (ol--put-buffer 'localRemote bufferLocalRemote)
      (ol--put-buffer 'localMerged bufferLocalMerged)
      (ol--put-buffer 'remoteMerged bufferRemoteMerged)
      ))

  (defun ol-merge-survival-knife-stop ()
    (interactive)
    ;; TOOD: Iterate instead
    (kill-buffer (ol--get-buffer 'base))
    (kill-buffer (ol--get-buffer 'local))
    (kill-buffer (ol--get-buffer 'remote))

    (kill-buffer (ol--get-buffer 'baseLocal))
    (kill-buffer (ol--get-buffer 'baseRemote))
    (kill-buffer (ol--get-buffer 'localRemote))
    (kill-buffer (ol--get-buffer 'localMerged))
    (kill-buffer (ol--get-buffer 'remoteMerged))
  ...
  (defun ol--ediff (bufferLeft bufferRight name)
  ...
  (defun ol--put-buffer (key buffer)
    (setq ol-merge-survival-knife-buffers (plist-put ol-merge-survival-knife-buffers key buffer)))

  (defun ol--get-buffer (key)
    (plist-get ol-merge-survival-knife-buffers key))
