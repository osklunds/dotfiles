...
#+begin_src emacs-lisp
  ...
  (defun msk-merge-survival-knife-start ()
    ...
    (let* ((file (magit-current-file))
	   ...
	   (bufferLocal  (msk--get-revision-buffer revLocal  fileLocal))
	   (bufferRemote (msk--get-revision-buffer revRemote fileRemote))
	   (bufferBase   (msk--get-revision-buffer revBase   fileBase))
	   (bufferMerged (current-buffer))

	   (bufferBaseLocal (msk--ediff bufferBase bufferLocal "BASE LOCAL"))
   	   (bufferBaseRemote (msk--ediff bufferBase bufferRemote "BASE REMOTE"))
	   (bufferLocalRemote (msk--ediff bufferLocal bufferRemote "LOCAL REMOTE"))
	   (bufferLocalMerged (msk--ediff bufferLocal bufferMerged "LOCAL MERGED"))
   	   (bufferRemoteMerged (msk--ediff bufferRemote bufferMerged "REMOTE MERGED")))
      
      ;; (message "EDIFF %s" bufferBaseLocal)

      (msk--put-buffer 'base bufferBase)
      (msk--put-buffer 'local bufferLocal)
      (msk--put-buffer 'remote bufferRemote)
      
      (msk--put-buffer 'baseLocal bufferBaseLocal)
      (msk--put-buffer 'baseRemote bufferBaseRemote)
      (msk--put-buffer 'localRemote bufferLocalRemote)
      (msk--put-buffer 'localMerged bufferLocalMerged)
      (msk--put-buffer 'remoteMerged bufferRemoteMerged)
      ))

  (defun msk-merge-survival-knife-stop ()
    (interactive)
    ;; TOOD: Iterate instead
    (kill-buffer (msk--get-buffer 'base))
    (kill-buffer (msk--get-buffer 'local))
    (kill-buffer (msk--get-buffer 'remote))

    (kill-buffer (msk--get-buffer 'baseLocal))
    (kill-buffer (msk--get-buffer 'baseRemote))
    (kill-buffer (msk--get-buffer 'localRemote))
    (kill-buffer (msk--get-buffer 'localMerged))
    (kill-buffer (msk--get-buffer 'remoteMerged))
  ...
  (defun msk--put-buffer (key buffer)
    (setq msk-merge-survival-knife-buffers (plist-put msk-merge-survival-knife-buffers key buffer)))

  (defun msk--get-buffer (key)
    (plist-get msk-merge-survival-knife-buffers key))
