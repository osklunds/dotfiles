...
#+begin_src emacs-lisp
  ...
  (defun ol-merge-survival-knife ()
    ...
    (let* ((file (magit-current-file))
      ...
      (message "Buffers: %s %s %s" bufferLocal bufferRemote bufferBase bufferMerged)

      (ol--ediff bufferBase   bufferLocal  "BASE   LOCAL")
      (ol--ediff bufferBase   bufferRemote "BASE   REMOTE")
      (ol--ediff bufferLocal  bufferRemote "LOCAL  REMOTE")
      (ol--ediff bufferLocal  bufferMerged "LOCAL  MERGED")
      (ol--ediff bufferRemote bufferMerged "REMOTE MERGED")
      ))
  ...
  (defun ol--ediff (bufferLeft bufferRight name)
    (let* ((bufferName (format "*Ediff Control Panel %s*" name))
           (rename-control-panel (lambda() (rename-buffer bufferName)))
	   (startup-hooks (cons rename-control-panel ()))
	   )
