...
(define-key evil-visual-state-map (kbd ", r") 'ol-full-replace-visual-selection)

(defun ol-full-replace-visual-selection ()
  (interactive)
  (ol-replace-visual-selection "%"))
...
(defun ol-replace-visual-selection (range)
  (let ((selection (buffer-substring-no-properties (mark) (point))))
    (ol-replace-selection selection range)))

(defun ol-replace-selection (selection range)
  (let ((ex-command (format "%ss/%s/%s/gc" range selection selection)))
