...
Before LGTM (let's get this merged):

- shell command and e.g. =--noignore= examples with completing-read
- fix compile errors
- move faces to only =ol-colors.el=
- don't require =ivy= or =counsel= from anywhere else
- investigte why =counsel-rg-base-command= isn't needed anymore
