...
Why this complexity? I've used =ivy= for a long time, but wanted to try =vertico= and friends, and it provides some features I didn't get to work with =ivy=, for example

- =embark='s quick pop out
- previews
- headings of =ripgrep= preserved

It also provided the benefit of forcing my to refactor out generic =completing-read= stuff from framework-specific.

Eventually I might consider writing my own framework for these reasons
